(function (root, factory) {
    ...
    function Typing(opts) {
        ...
        if (!(typeof this.opts.scrollTop == "function")) this.opts.scrollTop = function () {}
    }
    ...
    Typing.fn = Typing.prototype = {
        ...
        play: function (ele) {
            ...
            if (typeof curr === "string") {
                ...
                for (var i = 0; i < attrs.length; i++) {
                    var attr = attrs[i];
                    if (attr.name == "typejs" && attr.value == 'pause') { //todo 插入暂停
                        pause_btn = true;
                    }
        ...
        resume: function () {
            if (this._stop == false) return;  //在打字中了,不需要执行打字
            this._stop = false;
            this.play(this.chain)
        },
        scrollTop: function () { //打字地位的屏幕最下方,滑动取消定位
            if (this._stop) return;
            this.opts.scrollTop();
        }
    };
